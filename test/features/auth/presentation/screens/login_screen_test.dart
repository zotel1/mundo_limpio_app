...
///
/// [initialStatus] permite controlar el estado inicial de auth.
/// El mockRepo se usa para controlar login()/register() en cada test.
...
void main() {
  ...
  group('LoginScreen — R2.3: Validación client-side', () {
    testWidgets('email vacío debe mostrar error de validación',
        (tester) async {
      await tester.pumpWidget(createTestApp(authProvider));
      await pumpUntilSettled(tester);
    ...
    testWidgets(
        'validación debe BLOQUEAR la llamada a login() (R2.3)',
        (tester) async {
      await tester.pumpWidget(createTestApp(authProvider));
      await pumpUntilSettled(tester);
  ...
  group('LoginScreen — R3.1: Login exitoso', () {
    ...
    testWidgets('login exitoso debe llamar repo.login con credenciales',
      ...
      await tester.enterText(
          find.byType(TextFormField).at(0), 'usuario@test.com');
      await tester.enterText(
          find.byType(TextFormField).at(1), 'MiPass123!');
      await tester.tap(
          find.widgetWithText(ElevatedButton, 'Iniciar Sesión'));
      await pumpUntilSettled(tester);

      verify(() => mockRepo.login('usuario@test.com', 'MiPass123!'))
          .called(1);
    });
  });
  ...
  group('LoginScreen — R3.2/R3.3: Error en login', () {
    testWidgets('credenciales inválidas debe mostrar mensaje de error (R3.2)',
      ...
      await tester.enterText(
          find.byType(TextFormField).at(0), 'bad@test.com');
      await tester.enterText(
          find.byType(TextFormField).at(1), 'wrong');
      await tester.tap(
          find.widgetWithText(ElevatedButton, 'Iniciar Sesión'));
      await pumpUntilSettled(tester);
    ...
    testWidgets('error de red debe mostrar mensaje (R3.3)',
        (tester) async {
      ...
      await tester.enterText(
          find.byType(TextFormField).at(1), 'pass123');
      await tester.tap(
          find.widgetWithText(ElevatedButton, 'Iniciar Sesión'));
      await pumpUntilSettled(tester);
