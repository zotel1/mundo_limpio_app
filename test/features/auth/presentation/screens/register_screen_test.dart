...
void main() {
  ...
  group('RegisterScreen — R2.3: Validación client-side', () {
    ...
    testWidgets('contraseña corta debe mostrar error de validación',
      ...
      await tester.enterText(
          find.byType(TextFormField).at(1), 'ab');
      await tester.enterText(
          find.byType(TextFormField).at(2), 'ab');
      await tester.tap(
          find.widgetWithText(ElevatedButton, 'Crear Cuenta'));
      await pumpUntilSettled(tester);
    ...
    testWidgets('confirmación distinta debe mostrar error',
        (tester) async {
      await tester.pumpWidget(createTestApp(authProvider));
      await pumpUntilSettled(tester);
      ...
      await tester.enterText(
          find.byType(TextFormField).at(2), 'OtraPass456!');
      await tester.tap(
          find.widgetWithText(ElevatedButton, 'Crear Cuenta'));
      await pumpUntilSettled(tester);
    ...
    testWidgets(
        'validación debe BLOQUEAR la llamada a register() (R2.3)',
        (tester) async {
      await tester.pumpWidget(createTestApp(authProvider));
      await pumpUntilSettled(tester);
  ...
  group('RegisterScreen — R2.1: Registro exitoso', () {
    testWidgets(
      ...
      await tester.enterText(
          find.byType(TextFormField).at(0), 'nuevo@usuario.com');
      await tester.enterText(
          find.byType(TextFormField).at(1), 'SecurePass123!');
      await tester.enterText(
      ...
      expect(find.text('Registro exitoso. Iniciá sesión.'),
          findsOneWidget);
    });
    ...
    testWidgets('registro exitoso debe llamar repo.register con datos',
      ...
      verify(() => mockRepo.register('nuevo@usuario.com', 'SecurePass123!'))
          .called(1);
    });
  });
  ...
  group('RegisterScreen — R2.2: Error en registro', () {
    testWidgets('email duplicado debe mostrar mensaje de error (R2.2)',
      ...
      await tester.enterText(
          find.byType(TextFormField).at(0), 'existente@test.com');
      await tester.enterText(
          find.byType(TextFormField).at(1), 'SecurePass123!');
      await tester.enterText(
          find.byType(TextFormField).at(2), 'SecurePass123!');
  ...
  group('RegisterScreen — Navegación', () {
    testWidgets('link a login debe navegar a LoginScreen',
        (tester) async {
      await tester.pumpWidget(createTestApp(authProvider));
      await pumpUntilSettled(tester);
