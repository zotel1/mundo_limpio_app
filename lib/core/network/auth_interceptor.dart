// Interceptor de autenticaci├│n para Dio.
//
...
// que los requests se encolen autom├íticamente mientras el
// refresh est├í en progreso (R4.3 ÔÇö dedup de concurrentes).

import 'package:dio/dio.dart';

import '../storage/token_storage.dart';

/// Interceptor que maneja autenticaci├│n JWT autom├ítica.
///
...
class AuthInterceptor extends QueuedInterceptor {
  ...
  /// como excepci├│n no manejada si nadie lo captura. Lo envolvemos
  ...
  void onRequest(
    ...
    try {
      ...
      // El backend devolver├í 401 y el onError lo manejar├í
    }
  ...
  void onError(
    ...
    // (esto evitar├¡a loops infinitos de refresh)
    ...
    // y retry despu├®s (R4.3 ÔÇö dedup de concurrentes)
    if (_isRefreshing) {
    ...
    try {
      ...
      final tokens = await tokenStorage.readTokens();
      if (tokens == null) {
        // No hay refresh token para renovar ÔåÆ error de autenticaci├│n
        _isRefreshing = false;
        _safeNext(err, handler);
        return;
      }
      ...
      // Resolver sincr├│nicamente (sin Future.microtask) para que el error
      // queue del QueuedInterceptor procese los siguientes errores MIENTRAS
      // _isRefreshing sigue true. As├¡ los requests concurrentes (R4.3) se
      ...
      // se agregan durante el procesamiento (R4.3 ÔÇö drenado del error queue).
      // Por cada pending que resolvemos sincr├│nicamente, el error queue
      // puede encolar m├ís en _pendingRequests.
      while (_pendingRequests.isNotEmpty) {
        ...
        try {
          ...
          pending.handler.resolve(response); // Sincr├│nico ÔÇö drena error queue
        } catch (e) {
          _safeReject(
      ...
      // El refresh fall├│ ÔÇö limpiar tokens y propagar error (R4.2)
